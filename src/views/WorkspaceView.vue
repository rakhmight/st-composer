                ...
                <div class="workspace__content">
                    ...
                    <div v-if="currentTest.remarks" style="height: auto">
                        ...
                        >
                            ...
                            <span>{{ currentLang.additional[28] }}</span>
                            </v-tooltip>
                        </div>
                    </div>
...
export default {
    ...
    mounted(){
        ...
        // Loader
        this.loader.text = this.currentLang.additional[29] 
        const loaderTextInterval = setInterval(()=>{
            if(this.loader.counter<3){
                ...
                if(this.loader.step == 'load-from-store'){
                    ...
                    this.loader.text = this.currentLang.additional[30] 
                } else if (this.loader.step == 'prepared-env'){
                    this.loader.text = this.currentLang.additional[31] 
                }
            }
        }, 500)
    ...
    methods:{
        ...
        addQuestionsFromWordFile(questions){
            ...
            this.questionsCounter = questions.length + this.questionsCounter
            console.log(this.questionsCounter);

            this.mapQuestions.push(...questions)

            if(oldQuestionsLength==0){
                this.currentQuestion = this.questions[0]
            }

            this.currentTest.testInfo.totalQuestions += questions.length
            this.currentTest.testInfo.qwi += questions.length
        ...
        async saveProcess(params){
            if(params){
                if(params.route){
                    ...
                    if(params.newTest){
                        this.saveProcessFinally.ctx = this.currentLang.additional[32] 
                    } else if(params.route){
                        this.saveProcessFinally.ctx = this.currentLang.additional[33] 
                    }
                    this.blockAddQBtn = true
            ...
            if(!this.savingProcessLoop && !params.route){
                ...
                this.saveProcessFinally.ctx = this.currentLang.additional[34] 
                } else{
